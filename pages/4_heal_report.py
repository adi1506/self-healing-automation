...
st.set_page_config(page_title="Heal Report", layout="wide")
...
if url:
    ...
    if rescan:
        ...
        with st.spinner("Running self-heal scan..."):
            report = healer.heal(url, excel_manager)
        ...
        cols = st.columns(6)
        cols[0].metric("Total", report["total_elements"])
        cols[1].metric("Unchanged", report["unchanged"])
        cols[2].metric("Changed", report["changed"])
        cols[3].metric("New", report["new"])
        cols[4].metric("Removed", report["removed"])
        cols[5].metric("Unresolved", report.get("unresolved", 0))
