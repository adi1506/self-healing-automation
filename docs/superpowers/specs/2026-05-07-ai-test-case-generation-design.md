...
- **Full transparency** — every generated cell carries a provenance marker so the user knows what to verify.
...
[Test Data Manager page]
    ...
    └─► User clicks "AI Generate Test Cases"
        ...
        [TestCaseGenerator]
            ...
            │
            ▼
        [Negative case derivation] — one negative row per violatable constraint
            │
            ▼
        Populated grid with provenance markers
```
...
- **NegativeCaseDeriver** — given the same constraint metadata, mechanically emit one negative test case row per violatable constraint.
...
Negative case derivation runs over the same metadata:
...
- a parallel provenance map: `{field_name: "heuristic" | "ai" | "ai-fallback" | "manual"}`
...
4. If still invalid, fall back to the heuristic generator and tag the cell `ai-fallback`.
...
Above the data editor. When clicked:

1. Reads the current grid state (preserves any rows the user already filled).
2. If the grid is empty, generates a baseline set: one happy-path row + one negative row per violatable constraint.
...
Provenance display: cells generated by the AI layer get a subtle marker (asterisk suffix in the value or a separate visual indicator if Streamlit's data editor allows). Heuristic-generated cells show no marker. Manual user edits clear the marker.
...
| LLM returns invalid JSON twice | Fall back to heuristic; cell tagged `ai-fallback`. |
| LLM returns value violating DOM constraints twice | Fall back to heuristic; cell tagged `ai-fallback`. |
...
- `tests/test_ai_test_data.py`
  ...
  - Mocked client returning invalid JSON twice → falls back to heuristic, tags `ai-fallback`
  - Mocked client returning value violating constraint → re-prompts once, then falls back
...
None at design time. Implementation may surface questions about exact Streamlit `data_editor` styling for provenance markers; if the editor cannot show them inline, fall back to a separate "Provenance" expander section below the grid.
